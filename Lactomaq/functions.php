<?php
   ...
   //Limita la cantidad de caracteres 
	...
	function resize($url_interna, $ancho, $alto) {
	   $imgsrc_temp = matthewruddy_image_resize( $url_interna, $ancho, $alto,true,false);
	   if (!is_wp_error($imgsrc_temp)):
		   $imgsrc_temp = $imgsrc_temp['url'];
		   if ($imgsrc_temp != ""):
				echo $imgsrc_temp;
		   else:
				echo "http://placehold.it/".$ancho."x".$alto;
		   endif;
	   else:
			echo "http://placehold.it/".$ancho."x".$alto;
	   endif;
...
// TODO: Columna de imagen destacada en entradas
